  ...
  return (
    <main>
      <section className="relative overflow-hidden bg-char-2 pb-0 pt-20 text-white">
        ...
        <div className="relative z-[2] mx-auto grid max-w-site items-center gap-12 px-[30px] py-[82px] lg:min-h-[680px] lg:grid-cols-[1fr_.82fr] lg:gap-16 lg:py-[100px]">
          <Reveal>
            <Link href="/resources" className="group mb-8 inline-flex items-center gap-2 font-display text-[12px] font-semibold text-white/45 transition-colors hover:text-lime">
              <Arrow stroke="currentColor" className="rotate-180 transition-transform group-hover:-translate-x-1" /> All resources
            </Link>
            <span className="block font-display text-[11px] font-semibold uppercase tracking-[1.4px] text-lime">{article.category} · {article.readTime}</span>
            ...
            <div className="mt-8 flex flex-wrap items-center gap-x-3 gap-y-2 text-[12.5px] text-white/42">
              {article.author && <span className="font-semibold text-white/75">{article.author}</span>}
          ...
          <Reveal>
            <div className={`relative aspect-[4/3] overflow-hidden rounded-[24px] border border-white/15 bg-gradient-to-br ${article.grad} shadow-[0_35px_90px_rgba(0,0,0,.34)]`}>
              ...
              <span className="absolute bottom-5 left-5 rounded-full border border-white/20 bg-char/75 px-4 py-2 font-display text-[10px] font-semibold uppercase tracking-[1px] text-white backdrop-blur-md">AfriConnect perspective</span>
            </div>
          </Reveal>
        </div>
